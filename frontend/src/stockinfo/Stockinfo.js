...
const Stockinfo = React.memo(
  ({ ticker, singleTicker = false }) => {
    ...
    const eps = ticker.epsCurrentYear ? `$${ticker.epsCurrentYear.toFixed(2)}` : 'N/A';
    ...
    const mktVol = ticker.regularMarketVolume
      ...
      : 'N/A';
    const dividendYield = ticker.dividendYield ? `${ticker.dividendYield} %` : '0 %';
    ...
    if (!collapse)
      return (
        <div data-testid="resolved" className="Stockinfo">
          <h4>
            <span className="Stockinfo-name" title="Collapse" onClick={handleCollapse}>
              {ticker.longName}
            </span>
            <span
          ...
          <div>
            ...
            {Object.keys(currentUser).length ? (
              <span>
                {!added ? (
                  <Button className="Stockinfo-watchlist-btn" onClick={addToWatchlist} size="sm">
                    Add
                  </Button>
                ) : (
                  <Button
